...
def classify_decline_reason(reason: str, ticket_repo: str
    ...
    """Classify a prerequisite or a pointed Accept/body conflict.

    The second value is a normalized prerequisite reference or a repository
    text pointer. Unknown and ambiguous reasons keep the existing blocked
    path. Bare prerequisite issue numbers resolve against the ticket repo.
    """
    found = {
        match.group("ref")
    ...
    if len(found) != 1:
        ...
        return ("accept-body-conflict", pointer) if pointer else ("unknown", None)
...
def finish_declined(
    ...
    context = checkout_context(cwd)
    resolved = resolve_checkout_repo(context["root"], repo)
    decline_class, decline_target = classify_decline_reason(reason, resolved)
    ticket = fetch_ticket(resolved, context["number"])
    ref = ticket["ref"]
